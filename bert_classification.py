...
def get_ag_news(num_samples):
    ...
    return (
        pd.read_csv(train_csv_path, usecols=[
                    0, 2], names=["label", "description"])
...
if __name__ == "__main__":
    ...
    trainer = pl.Trainer.from_argparse_args(
        args, callbacks=[lr_logger, early_stopping,
                         checkpoint_callback], checkpoint_callback=True
    )
    trainer.fit(model, dm)
    trainer.test(datamodule=dm)

    mlflow_pyfunc_model_path = 'mlflow_pyfunc_model'
    os.makedirs(mlflow_pyfunc_model_path, exist_ok=True)

    model_path = os.path.join(mlflow_pyfunc_model_path, 'model.pt')
    tokenizer_dir = os.path.join(mlflow_pyfunc_model_path, 'tokenizer')
    params_path = os.path.join(mlflow_pyfunc_model_path, 'params.json')

    torch.save(model.state_dict(), model_path)
    dm.tokenizer.save_pretrained(tokenizer_dir)
    json.dump(dict_args, open(params_path, 'w'))

    artifacts = {
        'model_dir': mlflow_pyfunc_model_path,
    }
    ...
    client.transition_model_version_stage(
        model_name, version=versions[-1].version, stage='Production')
